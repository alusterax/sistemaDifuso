...
def updateNomes():
    global entradasNomes
    global saidasNomes

    newNomesEntradas = []
    newNomesSaidas = []
    for entrada in entradas:
        descrEntrada = entrada["descricao"]
        newNomesEntradas.append(descrEntrada)
    for saida in saidas:
        descrSaida = saida["descricao"]
        newNomesSaidas.append(descrSaida)
    entradasNomes = newNomesEntradas
    saidasNomes = newNomesSaidas

def updateBoxes():
    sizeInp = boxInp.size()
    sizeRules = boxRules.size()
    for i in range(sizeInp):
        boxInp.delete(0)
    for i in range(sizeRules):
        boxRules.delete(0)
    for name in entradasNomes:
        boxInp.insert(END, name)
    for rule in regras:
...
def addCampoSaida_click():
    inp,campo = comboAddCampoSaida.get(), enSaidaAtr.get()
    if (len(inp) > 0 and len(campo) > 0):
        todosCampos = []
        atr = copy.deepcopy([saida['atributos'] for saida in saidas if saida['descricao'] == inp][0])
        for idx,atributo in enumerate(atr):
            todosCampos.append(atributo['descricao'])
        if (campo not in todosCampos):
            novoAtributo = {
            ...
            for saida in saidas:
                if saida["descricao"] == inp:
                    saida["atributos"].append(new)
                    #pprint.pprint(saida)
...
def updateCboxSaidas(box):
    box['values'] = saidasNomes
...
def updateCboxVariaveisSaida(box):
    try:
        box['values'] = retornaAtributosSaida(comboSaida.get())
    except:
        box['values'] = ''
        print("Adicione campos para a variável de saída!")
...
def saveConfig():
    ...
    with open("config.json",'w') as saida:
        json.dump (c,saida)
    print('Saved!')
...
def loadConfig():
    try:
        ...
        entradas = dadosConfig['entradas']
        saidas = dadosConfig['saidas']
        regras = dadosConfig['regras']
        print('Loaded!')
        print('---Entradas')
        pprint.pprint(dadosConfig['entradas'])
        print('--Saidas--')
        pprint.pprint(dadosConfig['saidas'])
        print('--Regras')
        pprint.pprint(dadosConfig['regras'])
        print('---')
        updateNomes()
        updateBoxes()
        ##fazer dar update nos nomes!
        ##fazer dar update nos boxes!
    except:
...
btSaveConfig = Button(janela, width=10, text="Save Config", command=saveConfig)
btLoadConfig = Button(janela, width=10, text="Load Config", command=loadConfig)
...
comboSaida["postcommand"] = partial(updateCboxSaidas, comboSaida)
comboSaida.current(0)

comboCampoSaida = ttk.Combobox(frameAddRegras, values=retornaAtributosSaida(comboSaida.get()), width=15)
comboCampoSaida["postcommand"] = partial(updateCboxVariaveisSaida, comboCampoSaida)

#-----
comboAddCampoSaida = ttk.Combobox(frameAddAtributoOutput, values=saidasNomes, width=15)
comboAddCampoSaida["postcommand"] = partial(updateCboxSaidas, comboAddCampoSaida)
comboAddCampoSaida.current(0)
...
enSaidaAtr = Entry(frameAddAtributoOutput, width=20)
...
btAddAtributoOutput = Button(frameAddAtributoOutput, width=10, text="Add campo", command=addCampoSaida_click)

scrInp = Scrollbar(frameInputSelect, orient=VERTICAL)
scrRules = Scrollbar(frameListaRegras, orient=VERTICAL)

boxInp = Listbox(frameInputSelect, height=22)
scrInp.config(command=boxInp.yview)
scrInp.place(x=128,y=5,height=356)
...
btAddAtributoOutput.place(x=190, y=60)
enSaidaAtr.place(x=145,y=30)
comboAddCampoSaida.place(x=5, y=30)
...
btSaveConfig.place(x=5, y=835)
btLoadConfig.place(x=95, y=835)
